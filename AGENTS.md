...
| `@centient/events` | 0.3.0 | Typed event streaming with backpressure. AsyncIterable + callback fan-out, JSONL persistence/replay, configurable backpressure, optional structural-logger injection (`EventsLogger`) on `createEventStream()`/`fromJsonl()`/`createJsonlSubscriber()` with `@centient/logger` as the zero-config default. Opt-in size-based JSONL rotation (`rotation: { maxSizeBytes, maxFiles }` on `createEventStream()`/`jsonl()`/`createJsonlSubscriber()`, **default off**) — rename-not-copy-truncate at a flush boundary, boot-time rotation of an already-oversized file, `<file>.<UTC-stamp>[-N]` names with an anchored retention match so operator-made siblings are never pruned; rotation failures log through the injected logger and continue, never throw. Injectable `clock` seam for deterministic stamps. Factory: `createEventStream()`, `fromJsonl()` |
...
| `@centient/resilience` | 0.1.0 | Zero-dependency resilience primitives. Clock-injected circuit breaker (CLOSED/OPEN/HALF_OPEN with exponential backoff), token-bucket rate limiter, backoff-with-jitter (linear sdk-compatible + exponential; opt-in `jitter: "full"` AWS full-jitter mode — uniform `[0, cap)`, no floor, can be 0 — plus an `attempts`/`maxTotalDelayMs` cumulative-budget guard enforced at construction), `withRetry()` with an injectable `shouldRetry` classifier (`isTransientError` preset: timeouts + 5xx + network retryable; 4xx incl. 409 CAS + `ZodError` + unknown terminal; `createTransientErrorPredicate()` for variants), LRU/TTL/SWR cache, bounded-concurrency pool. Clock + randomness + sleep injected throughout (no `Date.now()`/`Math.random()` in logic paths). Factory: `createCircuitBreaker()`, `createTokenBucket()`, `createBackoff()`, `withRetry()`, `createCache()`, `createPool()` |
| `@centient/secrets` | 0.9.0 | Cross-platform secrets vault with AES-256-GCM encryption + AAD binding, session-backed envelope vault (`openVault`), monotonic-version + sidecar rollback protection. Keychain/libsecret/Windows Credential Manager/GPG file/env backends + scrypt passphrase key-provider (hidden TTY prompt) for keychain-less unlock. `rekeyVault()` re-encrypts a vault under a new master key with an all-or-nothing `commit` seam (rolls the ciphertext back if the caller's commitment fails) — backs `centient secrets migrate passphrase`, the ADR-001 Amendment 1 `setupKey()` route. `OnePasswordVault` (ADR-004) stores credential *values* in 1Password — opt-in only (never auto-selected; an explicit choice fails closed rather than substituting), values written over stdin so they never appear in argv, names-only list cache. The credential-key grammar is enforced once at the cascade (#168) — `storeCredential`/`getCredential`/`deleteCredential`/`listCredentials` throw typed `InvalidCredentialKeyError` for a non-conforming key instead of letting it store on keychain/env and be unreadable everywhere else; `listCredentials(prefix)` validates against the looser prefix grammar so `"soma.anthropic."` still works. Factory: `openVault()`, `rekeyVault()`, `storeCredential()`, `getCredential()`, `deleteCredential()`, `listCredentials()` |
| `@centient/sdk` | 2.4.0 | TypeScript SDK for Engram Memory Server REST API. 35 resource classes, 130+ types, `evidence` (ADR-042 D3 append-only evidence series — dedup-aware `append` + `get` + paginated `listBySeries`/`listByEntity`/`listByDescriptor`; `EvidenceRecord.seq` is a decimal string (BIGSERIAL), `append()` returns `{ record, isDuplicate, priorSeq }` for both the 201 and 200 outcomes, typed `EvidenceDedupConflictError` never last-write-wins), `invitations` (ADR-044 invite/provisioning/connection lifecycle), `expectedVersion` CAS + `skipEmbedding` on `crystals.update`/`crystals.create`, incremental `crystals.list()` (ADR-040 `updatedAfter`/`createdAfter` watermarks composed with the #925 keyset `cursor` → `nextCursor`; offset and cursor are a typed either/or, zone-less watermarks and a non-string wire cursor both fail loudly — offset callers unaffected), `maintenance.vacuum()`, `shimmers` (node-local TTL state — lock/heartbeat/ipc; typed `ShimmerCasConflictError`/`ShimmerDisabledError`), `consolidationEvents` (public consolidation lifecycle — `listBySession`/`listByStatus`/`get` reads + write-scoped `consolidate` dry-run\|live/`undo` 60-day soft-revert; retires the last `asInternal()` seam), sync resource aligned to the 0.34.0 `{success,data}` envelopes (NDJSON push/pull), `events.subscribeIter()` AsyncIterable streaming with bounded backpressure (deprecated `events.subscribe()` now gated behind explicit opt-in), public dedup/merge-review methods (`notes.dedup()`, `crystals.pendingMerges()`/`reviewMerge()`/`mergeHistory()`) retiring the `asInternal()` path, jittered retries + optional injected logger, injectable retry classifier (`shouldRetry` on `createEngramClient()` governs every request path — the 5xx gate, the transport catch, and the request timeout; `retries` still caps attempts and a deterministic non-JSON 2xx body is never re-issued; default `isRetryableError` **unchanged**, opt into the #116 brownout taxonomy with `isBrownoutTransientError` — timeouts retried, unknown errors not — the default flip is deferred to the next major). Factory: `createEngramClient()`. Requires engram-server >= 0.31.0 (vacuum + skipEmbedding-on-create need >= 0.34.0; shimmers need the `/v1/shimmers` surface + `ENGRAM_SHIMMER_ENABLED`; consolidationEvents needs >= 0.41.0; evidence needs >= 0.47.0; incremental crystal listing needs >= 0.45.0) |
