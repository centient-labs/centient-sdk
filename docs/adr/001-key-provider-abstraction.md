...
**Status:** Accepted (extension) — **security properties RE-AUDITED 2026-06-23** (both runtime-verified; see [Security-relevant properties](#security-relevant-properties--re-audited-2026-06-23) below). The one divergence found at that audit — `migrate --to passphrase` unimplemented (#122) — was **closed 2026-07-22**; the commitment below is now live.
**Date:** 2026-06-12
...
- **`storeKey()` is unsupported.** A passphrase provider derives the key; it does not store a caller-supplied random key. `storeKey()` returns `false` and records an `UNSUPPORTED_OPERATION` diagnostic. Vault bootstrap uses the optional **`setupKey()`** method on the `KeyProvider` interface (`types.ts:57`), which prompts twice (entry + confirmation), writes the metadata sidecar, and returns the derived key. Migration *to* passphrase must therefore route through `setupKey()`, not `storeKey()`. **Implemented (2026-07-22, closes #122):** `centient secrets migrate passphrase` now takes a rekey branch — `setupKey()` derives the new key and writes the metadata, then `rekeyVault()` (`vault/session-vault.ts`) re-encrypts the vault under it. The provider-only key-move path is unchanged for `keychain`/`1password`, where the same master key simply relocates.

  The ordering carries the safety property: `rekeyVault()` writes the new ciphertext atomically, then runs the caller's `commit` — which is where `secrets.provider` is written — and restores the prior ciphertext byte-for-byte if that write fails. There is therefore no reachable state in which the configured provider cannot open the vault, in either direction. A failed derive or a failed rekey also removes the passphrase metadata written during the attempt, so the operator lands exactly where they started. `rekeyVault()` enforces the same rollback and missing-sidecar refusals as `openVault()`: a rekey that accepted a rolled-back vault would re-publish stale secrets at a fresh version and launder the rollback past every later check. Legacy AAD-less vaults are upgraded to schema 1 in the same step.
...
### Security-relevant properties — RE-AUDITED (2026-06-23)

ADR-001 was affirmed at phase-2 spot-check (`docs/hardening/STATE.md`) **before** this provider existed, so the affirmation did not cover it. The two properties from `895b88d` below have now been through a scoped adversarial `cl-adr-audit` (audit → refutation). **Both survive refutation and are runtime-verified, not merely test-exercised** — the auditor's verdict that they were "test-exercised" was if anything an under-claim. No "built" claim was refuted. Each property carries its verification evidence inline below.

1. **64 KiB hidden-input cap.** `MAX_HIDDEN_INPUT_LENGTH = 65_536` (`cli/hidden-prompt.ts:52`) bounds accumulated hidden input; a hostile or broken input source feeding endless bytes with no submit cannot grow `state.input` without bound — the prompt throws `HiddenPromptError` and drops the reference. **Re-audit verdict (2026-06-23): VERIFIED.** The cap is enforced *inside* the read loop (`hidden-prompt.ts:143-148`) and is actually reached on the unbounded-input path — test `tests/hidden-prompt.test.ts:146` feeds `cap + 512` bytes and asserts both the throw and the terminal restore; `:172` accepts exactly the cap. Not dead behind an earlier return. The GC-residue caveat stands as documented (immutable JS string cannot be zeroed); the transient `readSync` byte buffer **is** zeroed (`hidden-prompt.ts:152`).
2. **Deferred-one-tick signal handling.** Because the prompt is synchronous, an external `SIGINT`/`SIGTERM`/`SIGHUP` received while `readSync` blocks is queued by libuv and dispatched only after the prompt unwinds. Listeners are installed to suppress the immediate default kill (which would leave the terminal in raw mode with echo off), and their removal is **deferred one tick** via `setImmediate` so the queued dispatch is not dropped (`cli/hidden-prompt.ts:114-160`). **Re-audit verdict (2026-06-23): VERIFIED at runtime.** A real child-process test (`tests/hidden-prompt.test.ts:218-270`) confirms the end-to-end behavior: `child.exitCode` is `null` while the prompt blocks, the prompt returns its value, a queued `SIGTERM` is then re-raised with `signal === 'SIGTERM'` reaching the default disposition, and `SIGNAL_SWALLOWED` is absent — i.e. the deferral demonstrably prevents the swallow it guards against. Handlers are installed via `process.once` and removed via `setImmediate(removeSignalHandlers)`.
