...
- dd1c8b6: Close two ADR-002 P0 audit gaps surfaced by the scoped `cl-adr-audit` re-run:
...
- acaa98b: Per-consumer vault keys (issue #80). Previously `KeychainProvider` hardcoded
...
- b171141: Add a passphrase key provider for headless vault unlock. The provider derives the vault key with scrypt from a typed passphrase plus per-vault salt, persists only KDF metadata plus an HMAC verifier beside the vault, and fails closed when no interactive TTY is available.
